...
const suggestionChips = document.querySelectorAll('.chip') as NodeListOf<HTMLButtonElement>;
...
function addMessage(content: string, type: 'user' | 'assistant'): HTMLDivElement {
  hideWelcomeScreen();
...
function updateStatus(status: 'connecting' | 'connected' | 'error', message: string): void {
  const dot = statusIndicator.querySelector('.status-dot') as HTMLSpanElement;
  const text = statusIndicator.querySelector('.status-text') as HTMLSpanElement;
